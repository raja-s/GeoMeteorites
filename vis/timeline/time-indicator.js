...
window.addEventListener('mouseup', event => {
    ...
    if (timeIndicatorHeld) {
        ...
        const LEFT_LIMIT  = window.innerWidth - TIMELINE_DIMENSIONS.WIDTH + TIMELINE_MARGINS.LEFT;
        const RIGHT_LIMIT = window.innerWidth - TIMELINE_MARGINS.RIGHT;
        
        time = xTimeline.invert(clamp(event.clientX, LEFT_LIMIT, RIGHT_LIMIT) - LEFT_LIMIT).getFullYear();
...
window.addEventListener('mousemove', event => {
    ...
    if (timeIndicatorHeld) {
        ...
        const BAR = document.getElementById('timeline-frequency-bars-group')
            .children[xTimeline.invert(X).getFullYear() - timelineStart()];
        const Y = TIMELINE_MARGINS.TOP  + TIMELINE_CHART_DIMENSIONS.HEIGHT -
            BAR.height.baseVal.value - TIME_HANDLE_HEIGHT - 2;
        
        timeIndicator.attr('transform', `translate(${X}, ${Y})`)
